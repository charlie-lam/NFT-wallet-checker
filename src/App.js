...
const App = () => {
  ...
  const fillCollectionsStats = async (partialCollections) => {
    const statCompleteCollections = await Promise.all(
      partialCollections.map(async (collection) => {
        ...
        return {
          ...collection,
          sevenDayVolume: collectionStats.sevenDayVolume,
  ...
  const handleWalletSubmit = async (address) => {
    ...
    const partialCollectionsArray = Object.entries(groupedNftObject).map(
      ([name, nfts]) => ({
        name,
        nfts,
      }),
    );
    const statFilledCollections = await fillCollectionsStats(
      partialCollectionsArray,
    );
