...
export const WalletSummary = ({ walletAddress, collections, ethPrice }) => {
  ...
  useEffect(() => {
    setTotalNfts(
      collections
        .map((collection) => collection.nfts)
        .reduce((acc, collectionNfts) => acc + collectionNfts.length, 0),
    );
    const filteredCollections = collections.filter(
      (collection) => collection.sevenDayVolume > 0.01,
    );
    setLiquidNfts(
      filteredCollections
        .map((collection) => collection.nfts)
